...
(function() {
  ...
  SQLiteUtilsPlugin = function(openargs, openSuccess, openError) {
    var dbname;
    if (!(openargs && openargs['name'])) {
      throw newSQLError("Cannot create a SQLiteUtilsPlugin db instance without a db name");
    }
  ...
  SQLiteUtilsPlugin.prototype.databaseFeatures = {
    isSQLiteUtilsPluginDatabase: true
  };

  SQLiteUtilsPlugin.prototype.openDBs = {};

  SQLiteUtilsPlugin.prototype.addTransaction = function(t) {
  ...
  SQLiteUtilsPlugin.prototype.transaction = function(fn, error, success) {
    if (!this.openDBs[this.dbname]) {
      error(newSQLError('database not open'));
      return;
    }
    this.addTransaction(new SQLiteUtilsPluginTransaction(this, fn, error, success, true, false));
  };

  SQLiteUtilsPlugin.prototype.readTransaction = function(fn, error, success) {
    if (!this.openDBs[this.dbname]) {
      error(newSQLError('database not open'));
      return;
    }
    this.addTransaction(new SQLiteUtilsPluginTransaction(this, fn, error, success, false, true));
  };

  SQLiteUtilsPlugin.prototype.startNextTransaction = function() {
    var self;
    self = this;
  ...
  SQLiteUtilsPlugin.prototype.abortAllPendingTransactions = function() {
  ...
  SQLiteUtilsPlugin.prototype.open = function(success, error) {
    ...
    if (this.dbname in this.openDBs) {
      ...
      step2 = (function(_this) {
        return function() {
          cordova.exec(opensuccesscb, openerrorcb, "SQLiteUtilsPlugin", "open", [_this.openargs]);
        };
      })(this);
      cordova.exec(step2, step2, 'SQLiteUtilsPlugin', 'close', [
        {
          path: this.dbname
        }
      ]);
  ...
  SQLiteUtilsPlugin.prototype.close = function(success, error) {
    if (this.dbname in this.openDBs) {
      ...
      cordova.exec(success, error, "SQLiteUtilsPlugin", "close", [
        {
          path: this.dbname
        }
      ]);
  ...
  SQLiteUtilsPlugin.prototype.executeSql = function(statement, params, success, error) {
    ...
    this.addTransaction(new SQLiteUtilsPluginTransaction(this, myfn, null, null, false, false));
  };

  SQLiteUtilsPlugin.prototype.sqlBatch = function(sqlStatements, success, error) {
    ...
    this.addTransaction(new SQLiteUtilsPluginTransaction(this, myfn, error, success, true, false));
  };

  SQLiteUtilsPluginTransaction = function(db, fn, error, success, txlock, readOnly) {
  ...
  SQLiteUtilsPluginTransaction.prototype.start = function() {
    var err;
    try {
      this.fn(this);
      this.run();
  ...
  SQLiteUtilsPluginTransaction.prototype.executeSql = function(sql, values, success, error) {
    if (this.finalized) {
      throw {
  ...
  SQLiteUtilsPluginTransaction.prototype.addStatement = function(sql, values, success, error) {
  ...
  SQLiteUtilsPluginTransaction.prototype.handleStatementSuccess = function(handler, response) {
    var payload, rows;
    if (!handler) {
      return;
    }
  ...
  SQLiteUtilsPluginTransaction.prototype.handleStatementFailure = function(handler, response) {
    if (!handler) {
  ...
  SQLiteUtilsPluginTransaction.prototype.run = function() {
    ...
    cordova.exec(mycb, null, "SQLiteUtilsPlugin", "backgroundExecuteSqlBatch", [
      {
        dbargs: {
  ...
  SQLiteUtilsPluginTransaction.prototype.abort = function(txFailure) {
  ...
  SQLiteUtilsPluginTransaction.prototype.finish = function() {
    var failed, succeeded, tx;
    if (this.finalized) {
      return;
    }
  ...
  SQLiteUtilsPluginTransaction.prototype.abortFromQ = function(sqlerror) {
    if (this.error) {
      this.error(sqlerror);
    }
  };
  ...
  SQLiteConnectionFactory = {
    ...
    openDatabase: argsArray(function(args) {
      ...
      return new SQLiteUtilsPlugin(openargs, okcb, errorcb);
    }),
    deleteDatabase: function(first, success, error) {
      ...
      delete SQLiteUtilsPlugin.prototype.openDBs[args.path];
      return cordova.exec(success, error, "SQLiteUtilsPlugin", "delete", [args]);
    }
  };
  ...
  SelfTest = {
    ...
    start: function(successcb, errorcb) {
      SQLiteConnectionFactory.deleteDatabase({
    ...
    step1: function(successcb, errorcb) {
      SQLiteConnectionFactory.openDatabase({
        name: SelfTest.dbname,
        location: 'default'
      }, function(db) {
        var check1;
  ...
  root.SQLiteUtilsPlugin = {
    sqliteFeatures: {
      isSQLiteUtilsPlugin: true
    },
    echoTest: function(okcb, errorcb) {
      ...
      return cordova.exec(ok, error, "SQLiteUtilsPlugin", "echoStringValue", [
        {
          value: 'test-string'
        }
      ]);
    ...
    openDatabase: SQLiteConnectionFactory.openDatabase,
    deleteDatabase: SQLiteConnectionFactory.deleteDatabase
  };
